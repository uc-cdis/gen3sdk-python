...
def summarize_tsvs(tsv_dir,dd,prefix='',outlier_threshold=3,omit_props=['project_id','type','id','submitter_id','case_submitter_id','md5sum','file_name','object_id'],omit_nodes=['metaschema','root','program','project','data_release'],outdir='data_summary_reports'):
    """
    ...
    data,nn_nodes,nn_props,null_nodes,null_props = {},[],[],[],[]
        ...
        for fname in fnames: # Each node with data in the project is in one TSV file so len(fnames) is the number of nodes in the project with data.
            ...
            if df.empty:
                ...
                for prop in props: #prop=props[0]
                    ...
                    nn = df.loc[df[prop]!='None'] # remove this after sheepdog bug fix is deployed:  https://ctds-planx.atlassian.net/browse/PXP-5663
...
def write_commons_report(summary,bin_limit=False,create_report=True,report_null=True,outdir='data_summary_reports',tsv_dir="."):
    ...
    report = pd.DataFrame(index=range(0,total_props),
        columns=['project','node','property','property_type',
                'N','nn','null','all_null',
