...
def testnewrecord(indexd):
    ...
    assert newrec['did'] == checkrec['did'] and \
           newrec['baseid'] == checkrec['baseid'] and \
           newrec['rev'] == checkrec['rev']
    ...
    updated = indexd.update_record(
            ...
            {'acl':['prog1', 'proj1'],
                ...
                'uploader':'mjmartinson'}).json()

    updatedrec = get_rec(indexd, updated['did'])
    # Note: I am not sure why the program and project are flipped!! 
    assert updatedrec['acl'] == ['proj1', 'prog1'] and \
           updatedrec['file_name'] == 'fakefilename' and \
           updatedrec['uploader'] == 'mjmartinson' and \
           updatedrec['did'] == checkrec['did'] and \
           updatedrec['rev'] != checkrec['rev']

    #delete the record
    response = indexd.delete_record(updatedrec['did'], updatedrec['rev']) 
    assert response.status_code == 200 and \
            no_record(indexd, updatedrec['did']) == True
#-------------------------------------------------------------------
@pytest.mark.skip
def testversions(indexd):
    ...
    return get_rec(indexd, newrec['did'])
    return get_rec(indexd, newversion['did'])
...
def testblank(indexd):
    ...
    assert newblank['did'] == checkrec['did'] and \
            newblank['baseid'] == checkrec['baseid'] and \
            newblank['rev'] == checkrec['rev']
    ...
    assert updatedblank['did'] == checkrec['did'] and \
            updatedblank['size'] == 1 and \
            updatedblank['did'] == {'md5': '4372834515237483626e387492837483'} \
            and updatedblank['rev'] != checkrec['rev']

    #delete the record
    response = indexd.delete_record(updatedblank['did'], updatedblank['rev']) 
    assert response.status_code == 200 and \
            no_record(indexd, updatedblank['did']) == True
#-------------------------------------------------------------------
