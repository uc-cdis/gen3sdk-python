...
def test_resolve_drs_with_indexd_dist(download_dir, index_dist, identifier, expected, write_cache):
    with requests_mock.Mocker() as m:
        m.get(f"https://dataguids_mock.org/index/_dist", json=index_dist)

        with mock.patch('gen3.tools.download.drs_resolvers.DRS_CACHE', str(
                Path(download_dir, ".drs_cache", "resolved_drs_hosts.json"))):
            results = resolve_compact_drs_using_indexd_dist(identifier,
...
def test_resolve_drs_with_indexd_dist_http_error(index_dist, identifier, expected):
    with requests_mock.Mocker() as m:
        m.get(f"https://dataguids_mock.org/index/_dist", json=index_dist, status_code=500)
        results = resolve_compact_drs_using_indexd_dist(identifier,
                                                        cache_results=False,
                                                        resolver_hostname="https://dataguids_mock.org"
                                                        )
...
def test_resolve_drs_from_local_cache_exceptions(download_dir):
    ...
    with mock.patch('gen3.tools.download.drs_resolvers.DRS_CACHE', str(
            Path(download_dir, ".drs_cache", "resolved_drs_hosts_not_found.json"))):
        results = resolve_drs_from_local_cache("dg.H35L", None)
        assert results is None

    # test with cache in un-parsable format
    with mock.patch('gen3.tools.download.drs_resolvers.DRS_CACHE', str(
            Path(DIR, "resources", "bad_format.json"))):
        results = resolve_drs_from_local_cache("dg.H35L", None)
        assert results is None
...
def test_resolve_compact_drs_using_dataguids():
    ...
    with requests_mock.Mocker() as m:
        m.get(f"https://dataguids_mock.org/index/{object_id}", json=DRS_RESOLVER_RESULTS)
        ...
        m.get(f"https://dataguids_mock.org/mds/metadata/{identifier}", json=DRS_MDS_RESULTS)
        results = resolve_compact_drs_using_dataguids(identifier, object_id,
                                                      resolver_hostname="https://dataguids_mock.org")
        expected = "test.commons1.io"
        assert results == expected
        ...
        m.get(f"https://dataguids_mock.org/mds/metadata/{identifier}", json=DRS_MDS_RESULTS)
        assert resolve_compact_drs_using_dataguids(identifier,
                                                   object_id,
                                                   resolver_hostname="https://dataguids_mock.org") == None
        ...
        assert resolve_compact_drs_using_dataguids(identifier, object_id,
                                                   resolver_hostname="https://dataguids_mock.org") is None
...
def test_resolve_using_mds(identifier, expected):
    ...
    with requests_mock.Mocker() as m:
        m.get(f"https://dataguids_mock.org/mds/metadata/{identifier}",
              json=MDS_RESPONSE.get(identifier, {
                  "detail": "Not found: dg.4503444"
              }), status_code=200 if identifier in MDS_RESPONSE else 404)

        results = resolve_drs_using_commons_mds(identifier, None,
    ...
    with mock.patch('gen3.tools.download.drs_resolvers.DRS_CACHE', str(
        ...
        with requests_mock.Mocker() as m:
            ...
            if object_id is not None:
                m.get(f"https://dataguids_mock.org/index/{object_id}", json=DATAGUIDS_MDS_RESPONSE[identifier])
            m.get(f"https://origin.mock.org/mds/metadata/{identifier}", json=DATAGUIDS_MDS_RESPONSE[identifier])

            results = resolve_drs(identifier, object_id,
