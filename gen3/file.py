...
class Gen3File:
    ...
    def download_multiple(
        ...
        guids = [
            item.get("guid") or item.get("object_id")
            for item in manifest_data
            if item.get("guid") or item.get("object_id")
        ]
        if not guids:
        ...
        with ThreadPoolExecutor(max_workers=num_parallel) as executor:
            ...
            for future in futures_iter:
                ...
                if result["status"] == "downloaded":
                    ...
                    logging.error(
                        f"Download failed for {result['guid']}: {result.get('error', 'Unknown error')}"
                    )

        logging.info(
            f"Download complete: {len(results['succeeded'])} succeeded, {len(results['failed'])} failed, {len(results['skipped'])} skipped"
        )
        return results
    ...
    async def async_download_multiple(
        ...
        try:
            ...
            if producer_thread:
                # Remove timeout limit for producer thread
            ...
            for p in processes:
                if p.is_alive():
                    p.terminate()
                    # Remove timeout limit for process cleanup
                    p.join()
                    if p.is_alive():
                        p.kill()
    ...
    async def _get_metadata(session, guid, endpoint, auth_token):
        ...
        try:
            # Use longer timeout for metadata requests
    ...
    async def _get_presigned_url(session, guid, endpoint, auth_token, protocol=None):
        ...
        try:
            # Use longer timeout for presigned URL requests
            async with session.get(
                api_url, headers=headers, timeout=aiohttp.ClientTimeout(total=60)
            ) as resp:
                if resp.status == 200:
    ...
    async def _download_content(session, url, guid, filepath):
        ...
        try:
            # Use no timeout limit for actual file downloads to handle large files
            async with session.get(
                url, timeout=aiohttp.ClientTimeout(total=None)
            ) as resp:
                if resp.status == 200:
                    # Stream download with large chunks for better performance
