...
@click.option('--filename-format', default='original', 
...
@click.pass_context
def download_single(ctx, guid, download_path, filename_format, protocol, 
                   skip_completed, rename, no_prompt, no_progress):
...
@click.option('--num-parallel', default=3, help='Number of parallel downloads')
@click.option('--skip-completed', is_flag=True, help='Skip files that already exist')
@click.option('--rename', is_flag=True, help='Rename files if they already exist')
@click.option('--no-prompt', is_flag=True, help='Do not prompt for confirmations')
@click.option('--no-progress', is_flag=True, help='Disable progress bar')
@click.pass_context
def download_multiple(ctx, manifest, download_path, filename_format, protocol,
                     num_parallel, skip_completed, rename, no_prompt, no_progress):
    ...
    try:
        ...
        # Confirm download if not no_prompt
        if not no_prompt:
        ...
        result = file_client.download_multiple(
        ...
        if result['failed']:
            ...
            for failure in result['failed']:
                click.echo(f"  - {failure['guid']}: {failure.get('error', 'Unknown error')}")
        
        success_rate = len(result['succeeded']) / len(manifest_data) * 100
        click.echo(f"\nSuccess rate: {success_rate:.1f}%")
        
        if result['failed']:
            raise click.ClickException("Some downloads failed")
            
    except Exception as e:
        logging.error(f"Batch download failed: {e}")
        raise click.ClickException(f"Batch download failed: {e}")
