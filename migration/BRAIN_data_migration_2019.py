...
# The get_followup fxn needs to be fixed:
def get_followup(project_id, node, label, visualization=1):
    ''' Get a table with summary statistics across different timepoints'''
    var = label
    # this query gets the first 10 cases in a "project_id"
    # then it gets all records in a specific "node", including the "var" for the "node"
    # finally, it gets the visit label for the visit associated with "node"
    query_text = """{ case(project_id: "%s"){
                        submitter_id
                        %s(first:0){
                            submitter_id
                            %s
    ...
    output = {}
    for fu in data['data']['case']:
        for m in fu['%s' % node]:
            for n in m["visits"]:
#                 print n["visit_label_other"]
                event_id = n["visit_label_other"]
...
#####
project_id = 'mjff-PPMI'
node = 'rem_sleep_behavior'
label = 'rbd_summary_score'
get_visit(project_id,node,label)

def get_visit(project_id, node, label, visualization=1):
    '''Get a table with summary statistics across different timepoints'''
    var = label
    query_text = """
{ visit (first:10,
...
""" % (project_id, node, node, var)
    print(query_text)
    data = query_api(query_text)

    # Extract data from query
    output = {}
    for visit in data['data']['visit']: # each visit is a dict:
        #keys: ['cases','rem_sleep_behaviors','submitter_id','visit_label','visit_label_other']
        event_id = visit['visit_label_other']
        visit[node+'s'][0][var]
        for m in visit[node+'s']: #may need to get the actual backref from dd
            output.setdefault(event_id, [])
            output[event_id].append(m[var])
...
{ rem_sleep_behavior (
        ...
        project_id:"mjff-PPMI",
        rbd_summary_score: 2,
        with_path_to:{type:"visit",visit_label_other:"Baseline"}
    ) {
        submitter_id
        rbd_summary_score
        visits (first:0) {
...
{ visit (first:10, project_id:"mjff-PPMI", with_path_to:{type:"rem_sleep_behavior",visit_label_other: "Baseline"}) {
    submitter_id
    visit_label
    visit_label_other
    cases (first:0) {
    ...
    rem_sleep_behaviors (first:0) {
        rbd_summary_score
        submitter_id
    }
}}

# Use case: get list of all case ids with a particular diagnosis

{
  diagnosis (first:0, primary_diagnosis:"Parkinson's Disease"){
    submitter_id
    primary_diagnosis
    visits {
      visit_label_other
...
# Use case: get all clinical assessment data for a particular case
{
